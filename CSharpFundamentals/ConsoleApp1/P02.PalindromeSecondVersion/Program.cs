namespace P02.PalindromeSecondVersion
{
    internal class Program
    {
        static void Main(string[] args)
        {
            string letters = "xhcchxau";
            int maxLen = 1;
            ...
            {
                string substring = letters.Substring(j, letters.Length - 1 - j);
                if (substring.Contains(letters[j]))
                {
                    int letterIndex = substring.IndexOf(letters[j]);
                    maxLen = Math.Max(maxLen, PalindromeLen(letters, j, letterIndex + 1));
                }
            }
